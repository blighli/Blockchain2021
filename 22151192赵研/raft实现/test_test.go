...
func TestBackup2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	DPrintf("Test:leader = [%d]",leader1)
	cfg.disconnect((leader1 + 2) % servers)
	cfg.disconnect((leader1 + 3) % servers)
	cfg.disconnect((leader1 + 4) % servers)
	DPrintf("Test:disconnect peer[%d,%d,%d]",(leader1 + 2) % servers,(leader1 + 3) % servers,(leader1 + 4) % servers)
	// submit lots of commands that won't commit
	DPrintf("Test:insert 50 commands to leader1[%d]",leader1)
	for i := 0; i < 50; i++ {
		temp := rand.Int()
		cfg.rafts[leader1].Start(temp)
		//DPrintf("Test:Command{%d}",temp)
	}

	DPrintf("Test:waiting %d ms",RaftElectionTimeout/2)
	time.Sleep(RaftElectionTimeout / 2)

	DPrintf("Test:disconnect peer[%d,%d]",(leader1 + 0) % servers,(leader1 + 1) % servers)
	cfg.disconnect((leader1 + 0) % servers)
	cfg.disconnect((leader1 + 1) % servers)

	DPrintf("Test:reConnect peer[%d,%d,%d]",(leader1 + 2) % servers,(leader1 + 3) % servers,(leader1 + 4) % servers)
	...
	DPrintf("Test:check agreement of %d peers",3)
	for i := 0; i < 50; i++ {
	...
	DPrintf("Test:disconnect peer[%d]",other)
	cfg.disconnect(other)

	// lots more commands that won't commit
	DPrintf("Test:append 50 commands to leader2[%d]",leader2)
	for i := 0; i < 50; i++ {
		temp := rand.Int()
		cfg.rafts[leader2].Start(temp)
		//DPrintf("Test:Command{%d}",temp)
	}
	...
	for i := 0; i < servers; i++ {
		cfg.disconnect(i)
		DPrintf("Test:disconnect peer [%d]",i)
	}
	cfg.connect((leader1 + 0) % servers)
	cfg.connect((leader1 + 1) % servers)
	cfg.connect(other)
	DPrintf("Test:reconnect peer[%d,%d,%d]",(leader1 + 0) % servers,(leader1 + 1) % servers,other)
	...
	DPrintf("Test:connect all peers")
	DPrintf("Test:check agreement of [%d] peers",servers)
	cfg.one(rand.Int(), servers, true)
...
func TestPersist12C(t *testing.T) {
	...
	DPrintf("Test(1):start insert one command.")
	cfg.one(11, servers, true)
	DPrintf("Test(1):cfg one")
	// crash and re-start all
	for i := 0; i < servers; i++ {
		cfg.start1(i, cfg.applier)
	}
	DPrintf("Test(1):disconnect and then connect all servers")
	for i := 0; i < servers; i++ {
		cfg.disconnect(i)
		cfg.connect(i)
	}
...
func TestFigure8Unreliable2C(t *testing.T) {
	...
	DPrintf("Test:insert one command to all")
	cfg.one(rand.Int()%10000, 1, true)

	nup := servers
	for iters := 0; iters < 1000; iters++ {
		//DPrintf("Test:loop [%d] times",iters)
		if iters == 200 {
			cfg.setlongreordering(true)
			DPrintf("Test:longrecording")
		}
		leader := -1
		for i := 0; i < servers; i++ {
			...
			if ok && cfg.connected[i] {
				leader = i
				DPrintf("Test:leader is [%d] and insert one command",leader)
			}
		}
		...
		if leader != -1 && (rand.Int()%1000) < int(RaftElectionTimeout/time.Millisecond)/2 {
			...
			DPrintf("Test:disconnect leader[%d]",leader)
			nup -= 1
		}
		...
		if nup < 3 {
			...
			if cfg.connected[s] == false {
				cfg.connect(s)
				DPrintf("Test:connect peer[%d]",s)
				nup += 1
			}
		}
	}

	DPrintf("Test:connect to all peers.")
	for i := 0; i < servers; i++ {
